                ...
                var calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    select: function(arg) { // Create Event
                        ...
                        Swal.fire({
                            ...
                            html:
                            '<input id="scheduletitle" class="swal2-input" placeholder="Schedule name" style="width: 84%;"  >' +
                            '<input id="scheduleday" class="swal2-input" placeholder="Schedule Day" style="width: 84%;"  >',
                            focusConfirm: false,
                            preConfirm: () => {
                                return [
                            ...
                            if (result.isConfirmed) {
                                ...
                                var scheduletitle = result.value[0].trim();
                                var scheduleday = result.value[1].trim();
                                var schedulestarttime = arg.startStr;
                                var scheduleendtime = arg.endStr;

                                if(Schedule_Title != '' && scheduleday != ''){

                                    // AJAX - Add event
                                    $.ajax({
                                        url: 'ajaxfile.php',
                                        type: 'post',
                                        data: {request: 'addEvent',Schedule_Title: scheduletitle, Schedule_Day: scheduleday, Schedule_StartTime: schedulestarttime, Schedule_EndTime: scheduleendtime},
                                        dataType: 'json',
